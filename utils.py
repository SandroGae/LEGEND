...
# Here are the functions for implementing the steel band
# Function to create a cuboid solid
def create_cuboid(name, x_length, y_length, z_length, registry):
    """Create a cuboid solid with specified parameters."""
    return pyg4ometry.geant4.solid.Box(name, x_length / 2, y_length / 2, z_length / 2, registry=registry)

# Function to create and place a physical volume
def place_cuboid(lv, name, mother_volume, position, registry, rotation=[0, 0, 0]):
    """Place a cuboid in the simulation world."""
    return pyg4ometry.geant4.PhysicalVolume(rotation, position, lv, name=name, motherVolume=mother_volume, registry=registry)
